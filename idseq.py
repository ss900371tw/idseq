...
def convert_text_to_fhir_structured_ai(patient_id, report_markdown, api_key):
    ...
    extraction_prompt = f"""
你是一個詳盡的臨床關鍵字與實體萃取引擎。
請分析以下的臨床巨基因體分析報告，並提取其中的醫療實體、病原體、病況、實驗室檢驗、觀察指標、症狀、醫療處置以及實際用藥。

[關鍵萃取規則]：
- 請勿提取僅被提及為假設性選項、文獻建議或經驗性舉例的藥物（例如在「應考慮使用 Doxycycline 或 Azithromycin 等藥物」或「例如 oseltamivir」這類句子中提到的藥物）。
- 唯有當這些臨床關鍵字代表該病患實際「存在、已診斷、已測量、實施中或被檢測到」的實體、疾病、藥物、症狀、微生物，或明確的臨床治療計畫時，才可以進行提取。
- 切勿產生幻覺或捏造未直接獲得報告文本支持的實體。
